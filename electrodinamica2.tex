...
%     DATOS DEL DOCUMENTO
% =========================
\newcommand{\TituloApunte}{electrodynAmic bullshIt}
...
En esta primera parte, el material queda concentrado principalmente en dos núcleos: la estructura local de la electrodinámica clásica y la obtención de las ecuaciones de onda electromagnéticas en el vacío.
...
\begin{align*}
    \E(\vec x, t) &= E_x\,\hat x + E_y \, \hat y + E_z \,\hat z,\\
    \B(\vec x, t) &= B_x\,\hat x + B_y \, \hat y + B_z \,\hat z.
\end{align*}
...
\begin{equation*}
    ...
    \left(\frac{\partial E_x}{\partial z}-\frac{\partial E_z}{\partial x}\right)\hat y+
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        %------------------------------------------------
        \def\theta{-18}      % dirección de propagación
        \def\L{2.75}         % longitud visible del frente
        \def\d{2.10}         % "profundidad" para dar aspecto de superficie
        \def\sep{3.80}       % avance del frente en dirección k

        %------------------------------------------------
        % Primer frente de fase
        %------------------------------------------------
        \coordinate (A) at (0,0);
        \coordinate (B) at ($ (A)+({\theta+90}:\L) $);
        \coordinate (D) at ($ (A)+(8:\d) $);
        \coordinate (C) at ($ (B)+(8:\d) $);

        %------------------------------------------------
        % Segundo frente de fase (mismo frente en t + Delta t)
        %------------------------------------------------
        \coordinate (s) at ({\theta}:\sep);
        ...
        %------------------------------------------------
        % Centros geométricos de los frentes
        %------------------------------------------------
        \coordinate (O1) at ($ (A)!0.5!(C) $);
        \coordinate (O2) at ($ (A2)!0.5!(C2) $);

        %------------------------------------------------
        % Frentes de onda
        %------------------------------------------------
        \fill[blue!7] (A)--(B)--(C)--(D)--cycle;
        \fill[blue!7] (A2)--(B2)--(C2)--(D2)--cycle;

        \draw[thick,blue!70!black] (A)--(B)--(C)--(D)--cycle;
        \draw[thick,blue!70!black] (A2)--(B2)--(C2)--(D2)--cycle;

        % Bordes "activos" del frente para enfatizar la fase
        \draw[very thick,blue!85!black] (A)--(B);
        \draw[very thick,blue!85!black] (A2)--(B2);

        %------------------------------------------------
        % Vector de propagación
        %------------------------------------------------
        \draw[very thick,-{Latex[length=4mm,width=2.6mm]},blue!85!black]
            (O1) -- (O2)
            node[midway,above=3pt,sloped] {$\vec k$};

        %------------------------------------------------
        % Etiquetas temporales
        %------------------------------------------------
        \node[font=\small,above left=2pt] at (B) {$t$};
        \node[font=\small,above left=2pt] at (B2) {$t+\Delta t$};
        ...
        {$\vec k\cdot\vec x-\omega t+\phi_0=\mathrm{cte}$};

        %------------------------------------------------
        % Distancia recorrida por el mismo frente
        %------------------------------------------------
        \coordinate (P1) at ($ (O1)+({\theta-90}:1.15) $);
        \coordinate (P2) at ($ (O2)+({\theta-90}:1.15) $);

        % pequeñas líneas auxiliares
        \draw[dashed,gray!65] (O1) -- (P1);
        \draw[dashed,gray!65] (O2) -- (P2);

        \draw[<->,gray!75!black] (P1) -- (P2)
            node[midway,below=2pt,sloped] {$\Delta x$};

        \node[
            font=\small,
            align=center
        ] at ($ (P1)!0.5!(P2)+(0,-0.62) $)
        {avance del mismo frente\\ de fase};
        ...
        {$v_f=\dfrac{\Delta x}{\Delta t}$};

    \end{tikzpicture}
    \endgroup
    \caption{Desplazamiento de una superficie de fase constante de una onda plana monocromática. Entre los instantes $t$ y $t+\Delta t$, el mismo frente de fase avanza una distancia $\Delta x$ en la dirección de propagación dada por $\vec k$. Esto permite identificar la velocidad de fase como $v_f=\Delta x/\Delta t$.}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        % ejes
        \draw[->] (O) -- (0,3.35) node[above] {$x$};
        \draw[->] (O) -- (-1.95,-1.45) node[left] {$y$};
        \draw[->] (O) -- (8.8,0) node[right] {$z$};
        ...
        % texto de fase constante
        \node[align=center] at (4.9,2.8)
            {frentes de onda\\[2pt]
            $\vec k\cdot \vec x-\omega t+\varphi_0=\mathrm{cte}$};

        % frentes de onda
        \foreach \z in {2.1,4.0,5.9,7.8}
        {
            \begin{scope}[shift={(\z,0)}]
                ...
                % flecha del campo eléctrico sobre cada frente
                \draw[thick,-{Latex[length=2.4mm]}]
                    (0.03,-0.02) -- (0.03,0.92);
            \end{scope}
        }

        % una flecha representativa de E
        \draw[very thick,-{Latex[length=2.8mm]}]
            (-0.9,0.2) -- (-0.9,1.95)
            node[midway,left] {$\vec E$};

        % una flecha representativa de B
        \draw[very thick,-{Latex[length=2.8mm]}]
            (1.75,-0.1) -- (0.95,-0.72)
            node[midway,below left] {$\vec B$};

        % longitud de onda
        \draw[<->] (2.1,-1.45) -- (4.0,-1.45)
            node[midway, below] {$\lambda$};

    \end{tikzpicture}
    \endgroup
    \caption{Representación geométrica de una onda plana monocromática que se propaga en la dirección $\vec k\parallel \hat z$. Los frentes de onda son planos de fase constante, y los campos $\vec E$ y $\vec B$ son transversales a la dirección de propagación.}
\end{figure}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        % ejes
        \draw[->] (-0.35,0) -- (\xmax+0.55,0) node[right] {$x$};
        \draw[->] (0,-2.05) -- (0,2.45) node[above] {$E_y$};

        % líneas de amplitud
        \draw[densely dashed] (0,\A) -- (\xmax,\A);
        \draw[densely dashed] (0,-\A) -- (\xmax,-\A);

        % perfil sinusoidal
        \draw[thick, smooth, samples=250, domain=0:\xmax]
            plot (\x, {\A*sin(360*\x/\lam + \phi)});

        % flechas del campo
        \foreach \n in {0,...,15}
        {
            \pgfmathsetmacro{\xx}{0.45 + 0.67*\n}
            \pgfmathsetmacro{\Ey}{\A*sin(360*\xx/\lam + \phi)}
            \draw[thick,-{Latex[length=2.2mm]}] (\xx,0) -- (\xx,\Ey);
        }

        % etiquetas laterales
        \node[left] at (0,\A) {$\vec E=\vec E_0$};
        \node[left] at (0,0) {$\vec E=\vec 0$};
        \node[left] at (0,-\A) {$\vec E=-\vec E_0$};

        % etiqueta temporal
        \node[fill=white, inner sep=2pt] at (5.7,1.95) {$t=t_0$};

        % texto explicativo fuera de la onda
        \node[
            fill=white,
            inner sep=3pt,
            align=left,
            anchor=west
        ] at (7.95,2.25) {\small en una dirección fija.};
        ...
        \node[
            ...
            align=left,
            anchor=west
        ] at (7.95,2.88) {\small Polarización lineal: $\vec E$ oscila siempre };
        ...
        \draw[
          ...
          node[midway, below=7pt] {$\lambda$};

    \end{tikzpicture}
    \endgroup
    \caption{Instantánea espacial del campo eléctrico de una onda plana linealmente polarizada. Aunque la magnitud del campo cambia con la posición, su dirección permanece fija.}
\end{figure}
...
\begin{equation*}
    \vec E_{01}= E_1\,\hat x \, , \qquad \vec E_{02}= E_2\, \hat y \, ,
\end{equation*}
entonces la amplitud de la superposición de las ondas será dada por
\begin{equation*}
    \vec E_{0T}= E_1\,\hat x + E_2\, \hat y.
\end{equation*}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=0.98,>=Latex,line join=round,line cap=round]
        \begin{scope}[shift={(0,0)}]
            \draw[->] (-2.0,0) -- (2.2,0) node[right] {$E_x$};
            \draw[->] (0,-2.0) -- (0,2.2) node[above] {$E_y$};
    ...
    \end{tikzpicture}
    \endgroup
    \caption{Caso en fase. A la izquierda se muestra la suma vectorial de las amplitudes $\vec E_{01}=E_1\hat x$ y $\vec E_{02}=E_2\hat y$, cuya resultante es $\vec E_{0T}$. A la derecha se muestra el lugar geométrico descrito por el extremo del campo eléctrico al fijar la posición y dejar correr el tiempo: una recta en el plano transversal.}
\end{figure}
...
\begin{align*}
    \vec E &= \operatorname{Re}\left[\vec E_{1,c}\,\ee^{\ii\left(\vec k\cdot\vec x-\omega t\right)}+\vec E_{2,c}\,\ee^{\ii\left(\vec k\cdot\vec x-\omega t\right)}\right] \\
    &= \operatorname{Re}\left[\left(E_0\hat x\,\ee^{\ii\varphi_1}\right)\ee^{\ii\left(\vec k\cdot\vec x-\omega t\right)}+\left(E_0\hat y\,\ee^{\ii\varphi_2}\right)\ee^{\ii\left(\vec k\cdot\vec x-\omega t\right)}\right] \\
    &= \operatorname{Re}\left[E_0\hat x\,\ee^{\ii\left(\vec k\cdot\vec x-\omega t+\varphi_1\right)}+E_0\hat y\,\ee^{\ii\left(\vec k\cdot\vec x-\omega t+\varphi_2\right)}\right].
\end{align*}
...
\begin{align*}
    \vec E
    &= \operatorname{Re}\left[E_0\hat x\,\ee^{\ii\left(\vec k\cdot\vec x-\omega t+\varphi_1\right)}+E_0\hat y\,\ee^{\ii\left(\vec k\cdot\vec x-\omega t+\varphi_1+\pi/2\right)}\right] \\
    &= \operatorname{Re}\left[E_0\left(\hat x+\ii\hat y\right)\ee^{\ii\left(\vec k\cdot\vec x-\omega t+\varphi_1\right)}\right] \\
    &= E_0\cos\left(\vec k\cdot\vec x-\omega t+\varphi_1\right)\hat x-E_0\sin\left(\vec k\cdot\vec x-\omega t+\varphi_1\right)\hat y.
\end{align*}
...
\begin{equation*}
    \vec E_{01}=E_{01}\,\hat x,
    \qquad
    \vec E_{02}=E_{02}\,\hat y,
    \qquad
    \delta=\varphi_2-\varphi_1.
\end{equation*}
Entonces la superposición puede escribirse como
\begin{equation*}
    \vec E(\vec x,t)=E_{01}\cos\left(\vec k\cdot\vec x-\omega t+\varphi_1\right)\hat x+E_{02}\cos\left(\vec k\cdot\vec x-\omega t+\varphi_1+\delta\right)\hat y.
\end{equation*}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=1.1, line cap=round, line join=round]
        ...
        \draw[->, thick] (O) -- (Kx) node[below right] {$k_y$};
        \draw[->, thick] (O) -- (Ky) node[below left] {$k_x$};
        \draw[->, thick] (O) -- (Kz) node[left] {$k_z$};
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        \draw[->,thick] (3.20,1.55) -- (5.00,1.55)
...
Consideremos ahora la \textbf{densidad de fuerza de Lorentz} escrita en notación indicial:
\begin{equation}
...
\end{equation}
y el \textbf{tensor de tensiones de Maxwell}
\begin{equation}
    T_{ij}(\vec x,t)
    :=
    \varepsilon_0E_iE_j
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[
        ...
        \begin{scope}[shift={(0,0)}]
            ...
            % Texto inferior
            \node[align=center,text width=5.6cm] at (0,-2.85)
        ...
        \begin{scope}[shift={(6.2,-0.1)}]
            ...
            % Fórmulas
            \node[align=left,text width=5.3cm] at (1.2,-2.05)
            {
    ...
    \end{tikzpicture}
    \endgroup
    \caption{Interpretación geométrica del término superficial en la ecuación integral de balance del momentum lineal electromagnético. Para un elemento orientado de superficie \(dS_j=n_j\,dS\) sobre la frontera \(\partial V\), el tensor de tensiones de Maxwell \(T_{ij}\) determina el flujo del componente \(i\) del momentum a través de dicho elemento, dado localmente por \(d\Pi_i^{(\mathrm{surf})}=T_{ij}dS_j\).}
    \label{fig:maxwell-stress-surface-flux}
\end{figure}
La figura \ref{fig:maxwell-stress-surface-flux} sugiere cómo debe interpretarse el término superficial en la ecuación integral: no como una abstracción puramente algebraica, sino como el flujo del componente \(i\) del momentum lineal del campo a través de cada elemento orientado de la frontera. En este sentido, el tensor \(T_{ij}\) desempeña para el momentum un papel análogo al que desempeña el vector de Poynting para la energía.

Con estas definiciones, \eqref{eq:momentum-balance-precompact} se compacta como
\begin{equation}
...
En el caso energético, la cantidad transportada es la energía electromagnética y el flujo está dado por el vector de Poynting. En el caso mecánico, la cantidad transportada es el momentum lineal del campo y el flujo asociado está codificado en el tensor de tensiones de Maxwell.

\section{Potenciales electromagnéticos y gauge}

% Aquí conviene insertar, cuando corresponda, el material de clases futuras sobre
% potencial escalar, potencial vector, transformaciones de gauge, calibre de Coulomb,
% calibre de Lorenz y ecuaciones para los potenciales.
...
\section{Comentario matemático sobre $L^2$, ortogonalidad y Fourier}
\label{ap:l2-fourier}
...
\begin{equation*}
    \vec E(\vec x,t)=E^1(\vec x,t)\,\hat x+E^2(\vec x,t)\,\hat y+E^3(\vec x,t)\,\hat z,
\end{equation*}
se supone que
\begin{equation*}
